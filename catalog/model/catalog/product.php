...
class ModelCatalogProduct extends Model {
	...
	public function updateInventory($inventory, $product_id) {
		$this->db->query("UPDATE " . DB_PREFIX . "product 
							 SET quantity = " . (int) $inventory['inventory'] . " 
						   WHERE product_id = " . (int) $product_id);
		
		$this->db->query("INSERT INTO " . DB_PREFIX . "product_attribute (product_id, attribute_id, language_id, text) 
							   VALUES (" . $product_id . ", 12, 1, '" . $this->db->escape($inventory['tor_inventory']) . "') 
			  ON DUPLICATE KEY UPDATE text = '" . $this->db->escape($inventory['tor_inventory']) . "'");

		$this->db->query("INSERT INTO " . DB_PREFIX . "product_attribute (product_id, attribute_id, language_id, text) 
							   VALUES (" . $product_id . ", 13, 1, '" . $this->db->escape($inventory['van_inventory']) . "') 
			  ON DUPLICATE KEY UPDATE text = '" . $this->db->escape($inventory['van_inventory']) . "'");
	}
}
