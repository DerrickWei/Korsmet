...
class ModelCheckoutOrder extends Model {
	...
	private $endpoint_quote = 'https://api.businesscentral.dynamics.com/v1.0/api/v1.0/companies(79e3a081-7ac2-45f7-998d-3d1b2e925d2a)/salesQuotes';

	private function getEnvironment() {
		if ($this->config->get('config_environment')) {
			// **** Development Environment **** //
			$this->endpoint_quote = 'https://api.businesscentral.dynamics.com/v1.0/api/v1.0/companies(41456175-baaa-4c6a-bbee-15c57159eedf)/salesQuotes';
		} else {
			// **** Production Environment **** //
			$this->endpoint_quote = 'https://api.businesscentral.dynamics.com/v1.0/api/v1.0/companies(79e3a081-7ac2-45f7-998d-3d1b2e925d2a)/salesQuotes';
		}
	}
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			// Send Order to Business Central as Sales Quote
			// $this->getEnvironment();

			$order_info = $this->getOrder($order_id);
			$products = $this->getOrderProducts($order_id); //echo "<pre>"; print_r($order_info); print_r($products); die("++++++");

			if ($order_info) {
				// Create new Log file
				$logger = new Log("new_order_log_" . date('Y-m-d H:i:s') . ".log");
				$logger->write('New order created . ');
				
				$sale_quote = array(
					'externalDocumentNumber'	=> 'OP - #' . $order_info['order_id'],
					'documentDate'				=> substr($order_info['date_added'], 0, 10),
					'customerId'				=> $order_info['payment_custom_field'][1],
					'customerNumber'			=> $order_info['payment_custom_field'][2],
					'currencyCode'     			=> 'CAD',
					'salesperson'				=> $order_info['custom_field'][3],
					'shippingPostalAddress'		=> array(
						'street'			=> $order_info['shipping_address_1'],
						'city'				=> $order_info['shipping_city'],
						'state'				=> $order_info['shipping_zone'],
						'countryLetterCode'	=> 'CA',
						'postalCode'		=> $order_info['shipping_postcode']
					)
				);

				$quote_from_bc = $this->Auth($this->endpoint_quote, 'POST', $sale_quote);
				$logger->write("Order data:" . print_r($quote_from_bc, true));

				foreach ($products as $product) {
					$product_details = $this->db->query("SELECT * 
					...
					$sales_quote_line = array(
						...
						'lineType'			=> 'Item',
						"quantity"			=> (int) number_format($product['quantity']),
						"unitPrice"			=> (float) number_format($product['price'], 2)
					);
					$logger->write("Sales Quote Line data:" . print_r($sales_quote_line, true));
