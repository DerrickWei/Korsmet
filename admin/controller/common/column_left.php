...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ($this->request->get['user_token'] == $this->session->data['user_token'])) {
			...
			/* Add Sync memu from Here */
			$sync = array();

			if ($this->user->hasPermission('access', 'sync/sync')) {
				$sync[] = array(
					'name'	   => $this->language->get('text_sync_items'),
					'href'     => $this->url->link('sync/sync', 'user_token=' . $this->session->data['user_token'], true),
					'children' => array()		
				);	
			}

			if ($this->user->hasPermission('access', 'sync/purchase_order')) {
				$sync[] = array(
					'name'	   => $this->language->get('text_purchase'),
					'href'     => $this->url->link('sync/purchase_order', 'user_token=' . $this->session->data['user_token'], true),
					'children' => array()		
				);	
			}
